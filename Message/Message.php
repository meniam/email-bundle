...
use Super\SuperEmailBundle\Event\RenderEvent;
use Super\SuperEmailBundle\SuperEmailEvent;
use Super\SuperEmailBundle\UserInterface;
...
class Message
{
    ...
    public function addSender($alias, Sender $sender)
    {
    ...
    public function render()
    {
        if (empty($this->rendered)) {
            $this->eventDispatcher->dispatch(SuperEmailEvent::BEFORE_RENDER_HTML, new RenderEvent($this));
            $this->renderSubject();
            $this->renderPlainTextBody();
            $this->renderHtmlBody();
            $this->eventDispatcher->dispatch(SuperEmailEvent::AFTER_RENDER_HTML, new RenderEvent($this));
            $this->rendered = true;
        } else {
    ...
    /**
     * @throws \Super\SuperEmailBundle\Exception\TemplateNotFoundException
     */
    ...
    {
        return $this->tpl.($this->locale ? '/'.$this->locale : '');
    }
    ...
    public function setPlainTextBody($plainBody)
    {
        $this->txtMessage = $plainBody;
    }
    ...
    public function getSwiftMessage()
    {
        ...
        $message->setId($this->getId()->toString().'@'.$this->getConfig()->getDomain());
